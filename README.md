...
首页顶部新增 `变更记录` 公告条：以一行摘要展示最近一次功能更新，点击 `查看全部` 弹出完整时间线对话框，回溯每一次功能更新，并在 `系统说明` 区块沉淀数据口径、隐私边界与部署方式等维护约定。公告条不占主界面，数据维护在 `data/changelog.json`，对话框内按日期倒序渲染并支持双区块切换。
...
页面顶部 `变更记录` 公告条展示最近一次更新摘要与总条数，点击 `查看全部`（或侧栏入口）弹出完整时间线对话框，数据来自 `data/changelog.json`：
